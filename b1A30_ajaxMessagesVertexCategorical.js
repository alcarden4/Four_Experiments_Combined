...
function sendRequestGet(){
	if(!document.getElementById('sync').checked){
		dhtmlxAjax.get("php/b1A30_processVertexCat.php?"+encodeURI(document.getElementById('params').value),outputResponse);
		alert("Request Sent");
	}else{
		var loader = dhtmlxAjax.getSync("php/b1A30_processVertexCat.php?"+encodeURI(document.getElementById('params').value));
		alert("Request Sent");
...
//				dhtmlxAjax.post("php/process.php","uid="+E.userid+"&key="+key+"&value="+value,outputResponse);
	uid =
		$.ajax({
			method: "POST",
			url: './php/b1A30_processVertexCat.php',
